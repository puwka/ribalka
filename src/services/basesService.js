...
function recordToForm(record) {
  ...
  return {
    ...
    work_hours: record.work_hours || record.workHours || '',
    imagesText: images.join('\n'),
    videosText: videos.join('\n'),
    type: record.type || 'paid',
    short_description: record.short_description || record.short || '',
...
function enrichRemote(row) {
  ...
  // Keep DB column names (short_description, price_label, work_hours, …)
  // so recordToForm / owner edit does not wipe them on save.
  return {
    ...row,
    ...mapped,
