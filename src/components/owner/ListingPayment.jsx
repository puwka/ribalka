    ...
    <div className="cabinet-panel listing-pay">
      ...
      <div className="listing-pay__card listing-pay__card--total">
        {!isUpgrade && renewQuote && (
          <>
            ...
            {renewQuote.topDays > 0 && (
              <div className="listing-pay__row">
                <span>ТОП {renewQuote.topDays} сут.</span>
                <strong>+{formatRub(renewQuote.topAmount)}</strong>
              </div>
            )}
          </>
        )}
      ...
      <div className="listing-pay__actions">
        <button
          type="button"
          className="btn-primary"
          disabled={paying || !ctor.enabled || (isUpgrade && amount === 0)}
          onClick={pay}
        >
          {paying
            ? 'Создаём платёж…'
