...
/** Separate mid-period TOP for directory (like photo upgrade for bases). */
export function buildDirectoryTopDailyQuery(topDays = 1) {
  return buildDirectoryPaymentQuery({
    mode: 'top_daily',
...
export default function DirectoryConstructorPricing({
  ...
  const minTopDays = topOnly ? 1 : 0;

  const quote = useMemo(
    () =>
      calcServiceTotal(t, {
        months: topOnly ? 0 : months,
        frame: topOnly || hideFrame ? false : frame,
        topDays: topSlotsFull ? 0 : topDays,
      }),
    [t, months, frame, topDays, topSlotsFull, topOnly, hideFrame]
  );

  const set = (patch) => {
    const next = { ...options, top: false, ...patch };
    if (topSlotsFull) next.topDays = 0;
    else if (topOnly && next.topDays != null) {
      next.topDays = Math.max(1, Math.min(90, Number(next.topDays) || 1));
    }
    onChange?.(next);
  };

  const topAmount = Math.round((topSlotsFull ? 0 : topDays) * dailyPrice);
      ...
      <div className="dir-pricing__body">
        ...
        <div className="dir-pricing__addons">
          ...
          {topSlotsFull ? (
            ...
            <div className="base-ctor__counters" style={{ marginTop: topOnly || hideFrame ? 0 : 8 }}>
              <div className="base-ctor__counter">
                ...
                <div>
                  <button
                    ...
                    onClick={() => set({ topDays: Math.max(minTopDays, topDays - 1) })}
                  >
                    −
                  </button>
        ...
        <div className="dir-pricing__total">
          {topOnly ? (
            <div className="dir-pricing__grand">
              <span>
                Итого за {topDays || 0}{' '}
                {topDays === 1 ? 'сутки' : topDays < 5 ? 'суток' : 'суток'}
              </span>
              <strong>{formatRub(topAmount)}</strong>
            </div>
          ) : (
            <>
              <div>
